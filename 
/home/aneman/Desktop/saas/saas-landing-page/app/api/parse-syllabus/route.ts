...
async function extractTextFromPPTX(file: File): Promise<string> {
  try {
    ...
    // Use mammoth to extract text from PPTX (it's actually a ZIP file)
...
async function callLLMToParseSyllabus(text: string): Promise<ParsedSyllabusData> {
  // For demo purposes, we'll use a rule-based parser if OpenAI API key is not available
  const hasApiKey = process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY !== "your-openai-key-here";
  ...
  try {
    const prompt = `
      Analyze this syllabus text and extract the following information in JSON format:
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert academic assistant that analyzes course syllabi and extracts important information for students. Always respond with valid JSON."
        },
        {
          role: "user",
          content: prompt
    ...
    console.error('OpenAI parsing failed, falling back to rule-based parser:', error);
    return parseSyllabusWithRules(text);
  }
}
